...
def get_point_flood_features(lat, lon, date_str):
    ...
    distToRiver = riverMask.fastDistanceTransform() \
        .sqrt() \
        .multiply(cellWidth) \
        .divide(1000) \
        .rename('dist_to_river')

    static_stack = ee.Image.cat([elevation, slope.rename('slope'), twi, distToRiver, landcover.rename('landcover')])
    ...
    def get_event_info():
        ...
        return ee.Dictionary({
            ...
            'duration': dur,
            'label_type': ee.Algorithms.If(ee.Number(dur).gt(5), 'High_Duration', 'Low_Duration')
        })

    event_data = ee.Dictionary(ee.Algorithms.If(has_event, get_event_info(), {
...
def get_flood_prediction(input_data, bundle_path='flood_model_bundle_V2.pkl', threshold=0.5):  #threshold = 0.436 or 0.596
    ...
    df['rain_std_7d'] = df[[f'rain_day_minus_{i}' for i in range(1,8)]].std(axis=1).fillna(0)
    df['rain_std_15d'] = df[[f'rain_day_minus_{i}' for i in range(1,16)]].std(axis=1).fillna(0)
    ...
    df['recent_max_intensity'] = df[['rain_day_minus_1', 'rain_day_minus_2', 'rain_day_minus_3']].max(axis=1)
    ...
    df['API_fast_7d'] = df[[f'rain_day_minus_{i}' for i in range(1,8)]].mul(weights_fast).sum(axis=1)
    ...
    # --- Forecast Features ---
    df['forecast_sum'] = df[['forecast_day_plus_0', 'forecast_day_plus_1', 'forecast_day_plus_2']].sum(axis=1)
    df['forecast_max'] = df[['forecast_day_plus_0', 'forecast_day_plus_1', 'forecast_day_plus_2']].max(axis=1)
