...
import { Header, FloodMap, FloodDetectionPanel } from './components';
...
export default function FloodMonitoring() {
  ...
  // State to control the 5-panel detection display
  ...
  // Update date when location changes (for real-time date tracking)
  useEffect(() => {
    if (selectedLocation && !selectedLocation.date) {
      // Update to current date when a location without fixed date is selected
      setCurrentDate(new Date().toISOString().split('T')[0]);
    } else if (customClickCoords) {
      // Update to current date when custom location is clicked
      setCurrentDate(new Date().toISOString().split('T')[0]);
    }
  }, [selectedLocation, customClickCoords]);

  // Show detection panel when detection result is available with images
  useEffect(() => {
    if (detectionResult && (detectionResult.images || detectionResult.image)) {
  ...
  const handleMonitor = () => {
    if (selectedLocation) {
      // Use location's fixed date if available, otherwise use current real-time date
      const dateToSend = selectedLocation.date || currentDate;
      monitorLocation(selectedLocation.coords[0], selectedLocation.coords[1], dateToSend);
    } else if (customClickCoords) {
      monitorLocation(customClickCoords[0], customClickCoords[1], currentDate);
    }
  };

  const handleCloseDetectionPanel = () => {
    setShowDetectionPanel(false);
  };

  // Get current coordinates for the detection panel
  const getCurrentCoordinates = (): [number, number] | undefined => {
    if (selectedLocation) return selectedLocation.coords;
    if (customClickCoords) return customClickCoords;
    return undefined;
=======
...
<<<<<<< HEAD
      ...
      {/* 5-Panel Flood Detection Display */}
